...
INSERT INTO Knowledge(knowledge_name,knowledge_text)
VALUES('ウイスキーの原料は？','ウイスキーの原料にはさまざまな穀物が使用されています。ジャパニーズウイスキー・スコッチウイスキー・アイリッシュウイスキーなどでは、発芽した大麦の麦芽を使用することが多いです。アメリカンウイスキーの場合はトウモロコシやライ麦・小麦や大麦など幅広く使用されています。ほかの原料にはオーツ麦・キビなどもあり、生産国それぞれの文化や伝統に基づいたルールに基づいて使用されています。');
INSERT INTO Knowledge(knowledge_name,knowledge_text)
VALUES('モルトウイスキーとグレーンウイスキーの違いは？','スコッチウイスキーには大きく分けてモルトウイスキーとグレーンウイスキーの2種類があり、原料や蒸留器、味わいに違いがあります。モルトウイスキーの原料は大麦を発芽させた麦芽。砕いて水と混ぜることで酵素が働き、デンプンを糖化します。これを濾過した麦汁を発酵させたアルコール度数約7％の液体を「単式蒸留器」という蒸留器で2回、もしくは3回蒸留。蒸留直後の原液は、「ニューポット(ニューメイク)」と呼ばれ、無色透明、アルコール度数60～70％です。そしてその原液を樽熟成します。
...
INSERT INTO Knowledge(knowledge_name,knowledge_text)
VALUES('ウイスキーと焼酎の違いは？','穀物から造られる蒸留酒という点では焼酎とウイスキーは似ていますが、焼酎は麹によってアルコールを発生させ、ウイスキーは穀物の糖化酵素を用い、さらに酵母を添加することによってアルコールを発生させるという違いがあります。また焼酎も樽熟成されることがありますが、着色度はウイスキーの5分の1程度まででなければならないと規制されています。');
INSERT INTO Knowledge(knowledge_name,knowledge_text)
VALUES('ウイスキーを熟成するのはなぜ？','無色透明の「ニューポット(ニューメイク)」は、樽での熟成を経て琥珀色に変化していきます。熟成することで樽からアルコールが蒸散し、非常にマイルドな味わいになり、複雑で豊かな香りの成分が出てきます。
...
INSERT INTO Knowledge(knowledge_name,knowledge_text)
VALUES('ウイスキーのアルコール度数は？','「ニューポット(ニューメイク)」のアルコール度数は60～70％程度ですが、一般的には加水することで40～43％程度で販売されることが多いです。アルコールの刺激を抑えることで鼻が香りを感じ取りやすく、ストレート・ロック・ハイボールと、さまざまな飲み方でバランス良く楽しめるといわれています。
アルコール度数は蒸留所の考え方でも異なり、たとえば日本最小の蒸留所として有名な滋賀県の長濱蒸溜所のウイスキーはモルトの風味が強いので、その力強さを表現するためには高めのアルコール度数が適していると考えて47％以上にこだわっています。');
INSERT INTO Knowledge(knowledge_name,knowledge_text)
VALUES('ウイスキーは低カロリー？','ウイスキーにもカロリーはありますが、ワインやビール・日本酒などの醸造酒と比べると低いといえるでしょう。糖質やプリン体を気にして、晩酌をビールからハイボールに変えたという方も多いようです。');
INSERT INTO Knowledge(knowledge_name,knowledge_text)
VALUES('ウイスキーのブレンドって？','単一の樽で熟成されたウイスキーをボトリングした「シングルカスク」「シングルバレル」を除き、ウイスキーはブレンドにより造られます。ブレンドが必要なウイスキーとして主に、モルト原酒とグレーン原酒をブレンドした「ブレンデッドウイスキー」、複数の蒸留所のモルト原酒をブレンドした「ブレンデッドモルト」、単一蒸留所のモルト原酒をブレンドした「シングルモルト」の3種類。ブレンドをすることで、ロットに関わらず品質を均一に保つことと、一方で新しい味わいを生み出すことが可能になります。
...
INSERT INTO Knowledge(knowledge_name,knowledge_text)
VALUES('世界5大ウイスキーとは？','現在では世界中でウイスキーが造られていますが、世界的に高い評価を受ける生産国として世界5大ウイスキーが知られています。ちなみにウイスキーの綴(スペル)は、アイルランドとアメリカではwhiskeyと表記されることが多く、そのほかの生産国ではwhiskyと表記されることが一般的です。
【スコッチウイスキー】
...
INSERT INTO Knowledge(knowledge_name,knowledge_text)
VALUES('ジャパニーズウイスキーはなぜ人気？','大手メーカーだけでなくクラフトウイスキーのメーカーも急増し、国際的にも評価が高まり続けるのがジャパニーズウイスキーです。蒸留所の規模によって、それぞれ強みがあります。大手メーカーの場合、一社で何種類もの酒質を造り分けることができ、そのバラエティー豊かな酒質をブレンドした多様なウイスキーが造れます。一方、小規模なクラフトウイスキーのメーカーは、風土を活かした酒造りと、フットワーク軽くさまざまな挑戦がしやすいことがメリットです。
...
INSERT INTO Knowledge(knowledge_name,knowledge_text)
VALUES('ウイスキーと食事のマリアージュは？','ウイスキーのおつまみといえばチョコレート・ドライフルーツ・ナッツと合わせることが一般的ですが、「BAR 新宿ウイスキーサロン」では、たとえば焼きマシュマロや、ピクルス・柚子ジャム・ミントを乗せたクラッカーなど、ウイスキーと合わせることで相乗効果が楽しめるフードの提案に力を入れています。
...
;
INSERT INTO Knowledge(knowledge_name,knowledge_text)
VALUES('「果実酒」と「甘味果実酒」の違いは？','果実酒とは、酒税法による酒の分類の一つで、果実を原料として発酵させたものと規定されており、具体的には、ワイン(原料果実：ブドウ)、シードル(原料果実：リンゴ)等があります。甘味果実酒とは、果実酒にブランデー等の蒸留酒を添加し、アルコール濃度を上げたもの(酒精強化)や、果実酒に薬草等を浸漬させたものが一般的です。前者の代表的なものとして、スペインの「シェリー」、ポルトガルの「ポート」、「マディラ」、イタリアの「マルサラ」が挙げられます。また後者としては、イタリアの「ヴェルモット」が有名です。');
